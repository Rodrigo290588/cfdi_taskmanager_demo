// ============================================================
// DrilldownDialogUuidDuplicadosNom.tsx · FASE C KPI#4
// Client Boundary Drilldown KPI #4 "UUID Duplicados" Nómina.
//
// Patrón 100% alineado a DrilldownDialogCanceladosNom (piloto):
// · Dialog fullscreen !w-screen !h-screen shadcn
// · Sticky thead + fila Input filtros x columna
// · Paginación cliente 200/página
// · Fila "Total Filtrado" Σ Per/Ded/Ot/Neto
// · Export CSV BOM UTF-8 filename sanitizado companyRfc
// · 3 EmptyStates (0 rows / fetch error / búsqueda vacía)
// · Skeleton Loader2
//
// Diferencias respecto a Cancelados (KPI específico):
// · Badge amber BOLD "Apariciones" (# veces aparece UUID repetido)
// · Fila destacada bg-amber-50 si apariciones >= 3 (riesgo alto)
// · Header: "{countUuidsUnicosDuplicados} UUIDs únicos dup · {countFilas} filas"
// · Columna CURP eliminada (no en route.ts endpoint)
// · Title icon = CopySlash (twin KpiCard)
// · EmptyState específico: "Sin UUIDs duplicados en el periodo"
// ============================================================
'use client';
...
import {
    Download, FileWarning, Loader2, CopySlash,
} from 'lucide-react';
...
export const DRILLDOWN_UUID_DUP_PAGE_SIZE = 200;

export type DrilldownUuidDupRow = {
    receiptId: string | null;
    invoiceId: string;
    uuid: string;
    aparicionesUuid: number;
...
export type DrilldownUuidDupResponse = {
    ok: boolean;
    generatedAt?: string;
    scope?: {
        companyId: string;
    ...
    totals?: {
        countUuidsUnicosDuplicados: number;
        countFilas: number;
    ...
    data?: DrilldownUuidDupRow[];
    error?: string;
    correlationId?: string;
    helpText?: string;
};

export type UuidDupDrilldownFilters = Partial<Record<
    | 'uuid'
    | 'aparicionesUuid'
    | 'fechaEmision'
    | 'fechaPago'
...
function _match(row: DrilldownUuidDupRow, f: UuidDupDrilldownFilters): boolean {
    const checks: Array<[unknown, string | undefined]> = [
        [row.uuid, f.uuid],
        [String(row.aparicionesUuid), f.aparicionesUuid],
...
function _uuDupBadgeVariant(aparicionesUuid: number): 'default' | 'destructive' | 'secondary' | 'outline' {
    if (aparicionesUuid >= 4) return 'destructive';
    if (aparicionesUuid >= 3) return 'outline';
    return 'secondary';
}

type DrilldownDialogUuidDuplicadosNomProps = Readonly<{
    open: boolean;
...
export function DrilldownDialogUuidDuplicadosNom(props: DrilldownDialogUuidDuplicadosNomProps) {
    const [loading, setLoading] = useState(false);
    const [rows, setRows] = useState<DrilldownUuidDupRow[]>([]);
    const [serverTotals, setServerTotals] = useState<DrilldownUuidDupResponse['totals'] | null>(null);
    const [errorMsg, setErrorMsg] = useState<string | null>(null);
    const [filters, setFilters] = useState<UuidDupDrilldownFilters>({});
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            setErrorMsg(null);
            try {
                const url = new URL('/api/dashboard/nomina/fiscal/drilldown/uuid_duplicados', window.location.origin);
                ...
                if (!res.ok) {
                    try {
                        const j = await res.json() as DrilldownUuidDupResponse;
                ...
                const json = await res.json() as DrilldownUuidDupResponse;
                ...
                console.error('[DrilldownDialogUuidDuplicadosNom] fetch error:', msg);
            } finally {
    ...
    const totalPages = Math.max(1, Math.ceil(filtered.length / DRILLDOWN_UUID_DUP_PAGE_SIZE));
    const safePage = Math.min(Math.max(1, page), totalPages);
    const paginated = useMemo(
        () => filtered.slice((safePage - 1) * DRILLDOWN_UUID_DUP_PAGE_SIZE, safePage * DRILLDOWN_UUID_DUP_PAGE_SIZE),
        [filtered, safePage],
    );
    ...
    function updateFilter<K extends keyof UuidDupDrilldownFilters>(k: K, v: string) {
    ...
    function handleExportCsv() {
        try {
            const headers = [
                '#Apariciones UUID','UUID','Fecha Emisión','Fecha Pago','Tipo Nómina','Serie','Folio','Estatus SAT',
            ...
            a.download = sanitizeDownloadFilename(
                `Reporte_UUIDs_Duplicados_Nomina_${suffix}_${ts}`,
                'Reporte_UUIDs_Duplicados_Nomina',
                'csv',
            );
            ...
            const msg = error_ instanceof Error ? error_.message : String(error_);
            console.error('[DrilldownDialogUuidDuplicadosNom] CSV export error:', msg);
        }
    }

    type _UuDupUIState = Readonly<{
        loading: boolean;
        errorMsg: string | null;
        rows: DrilldownUuidDupRow[];
        paginated: DrilldownUuidDupRow[];
        filtered: DrilldownUuidDupRow[];
    }>;
    type _UuDupUIControls = Readonly<{
        filters: UuidDupDrilldownFilters;
        filteredTotals: typeof filteredTotals;
        updateFilter: <K extends keyof UuidDupDrilldownFilters>(k: K, v: string) => void;
    }>;

    function _uuDupRenderLoading() {
        return (
            <div className="flex-1 flex justify-center items-center gap-2 text-muted-foreground p-8">
                <Loader2 className="h-6 w-6 animate-spin" />
                <span className="text-sm">Cargando reporte UUIDs duplicados...</span>
            </div>
        );
    }
    function _uuDupRenderErrorState(errorMsg: string) {
        return (
    ...
    function _uuDupRenderEmptyState() {
        return (
            <div className="flex-1 flex justify-center items-center p-8">
                <FiscalNomEmptyState
                    icon={<CopySlash className="h-10 w-10 text-amber-500" />}
                    title="Sin UUIDs de nómina duplicados en el periodo"
                    subtitle="No se detectaron recibos cuyo UUID aparezca más de 1 vez para esta empresa y rango."
                    hint="Cambia el rango de fechas o valida la importación de CFDIs."
                />
            </div>
        );
    }
    function _uuDupRenderTable(ui: _UuDupUIState, c: _UuDupUIControls) {
            ...
            <Table className="w-full min-w-max">
                <TableHeader className="sticky top-0 bg-muted z-10 shadow-sm">
                    ...
                    <TableRow className="bg-muted/50 border-b shadow-sm">
                        <TableHead className="p-1 px-2 align-top"><Input placeholder="Buscar..." className="h-7 text-xs bg-background text-center" value={filters.aparicionesUuid || ''} onChange={e => updateFilter('aparicionesUuid', e.target.value)}/></TableHead>
                ...
                <TableBody>
                    {paginated.map((r, i) => {
                        const highlight = r.aparicionesUuid >= 3;
                        const badgeVariant = _uuDupBadgeVariant(r.aparicionesUuid);
                        return (
                            <TableRow
                                key={`${r.uuid}-${r.receiptId || r.invoiceId}-${i}`}
                                className={highlight ? 'bg-amber-50 hover:bg-amber-50 dark:bg-amber-950/20' : undefined}
                            >
                                <TableCell className="whitespace-nowrap text-center">
                                    <Badge variant={badgeVariant} className="font-bold text-xs px-2">
                                        x{fmtNum(r.aparicionesUuid, 0)}
                                    </Badge>
                                </TableCell>
                    ...
                    {filtered.length === 0 && (
                        <TableRow>
                            <TableCell colSpan={13} className="h-24 text-center text-muted-foreground text-sm">
                    ...
                    {filtered.length > 0 && (
                        <TableRow className="bg-muted/50 font-bold hover:bg-muted/50">
                            <TableCell colSpan={10} className="text-right text-xs">
                                Total Filtrado · {fmtNum(filtered.length)} fila(s) ·{' '}
                                {fmtNum(new Set(filtered.map(r => r.uuid)).size)} UUID(s) únicos
                            </TableCell>
    ...
    function renderMainContent(
        ...
        if (ui.loading) return _uuDupRenderLoading();
        if (ui.errorMsg) return _uuDupRenderErrorState(ui.errorMsg);
        if (ui.rows.length === 0) return _uuDupRenderEmptyState();
        return _uuDupRenderTable(ui, c);
    }
                ...
                <DialogHeader className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 shrink-0 pr-8">
                    <div className="space-y-1 min-w-0">
                        <DialogTitle className="text-base sm:text-lg font-semibold flex items-center gap-2">
                            <CopySlash className="h-5 w-5 text-amber-500" />
                            Reporte · UUIDs de Nómina Duplicados
                        </DialogTitle>
                            ...
                            {serverTotals && (
                                <div className="text-[11px]">
                                    ...
                                    <strong className="text-amber-700 font-semibold">
                                        {fmtNum(serverTotals.countUuidsUnicosDuplicados)}
                                    </strong>{' '}
                                    UUIDs únicos duplicados ·{' '}
                                    {fmtNum(serverTotals.countFilas)} filas ·{' '}
                ...
                {!loading && filtered.length > 0 && (
                    <div className="mt-0 shrink-0 flex items-center justify-between gap-3 text-sm text-muted-foreground">
                        <div className="text-xs">
                            Mostrando{' '}
                            {((safePage - 1) * DRILLDOWN_UUID_DUP_PAGE_SIZE) + 1}-
                            {Math.min(safePage * DRILLDOWN_UUID_DUP_PAGE_SIZE, filtered.length)}{' '}
