...
export async function POST(request: NextRequest) {
  try {
    ...
    async function processXml(xml: string) {
      ...
      // Validación adicional para emitidos: el emisor debe coincidir con la empresa seleccionada
      if (issuerRfc.toUpperCase() !== companyRfc.toUpperCase()) {
        return results.push({ uuid, status: 'error', message: 'El RFC emisor del XML no coincide con la empresa seleccionada' })
      }
