...
import { listPaymentMethodVsPaymentFormRuleViolations } from '@/lib/provider-business-rules'
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!canAccessReceptionBusinessRulePueForma99(access)) {
    ...
    const rows = await listPaymentMethodVsPaymentFormRuleViolations({
    ...
    console.error('Dashboard recibidos business rules drilldown API error:', error)
