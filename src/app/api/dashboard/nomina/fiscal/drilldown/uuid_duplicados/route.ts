// ============================================================
// src/app/api/dashboard/nomina/fiscal/drilldown/uuid_duplicados/route.ts
// FASE B · Drilldown KPI #4 "UUID Duplicados" · Tablero Fiscal Nómina.
//
// Endpoint de SOLO LECTURA (SELECT) — Regla 23 AGENTS.md cumplida.
// Retorna TODOS los CFDIs de NÓMINA cuyo UUID aparece más de 1 vez
// en el rango de fechas para la empresa (riesgo doble timbrado SAT).
//
// Hardening (copia fiel cancelados/route.ts para consistencia):
//   · Zod strict + regex allow-list IDs custom/CUID2 (DASH-SAST-009).
...
const UUID_DUP_PAGE_SIZE = DRILLDOWN_DEFAULT_PAGE_SIZE;
const ID_REGEX = DRILLDOWN_ID_REGEX;
const MAX_RANGO_DIAS = DRILLDOWN_MAX_RANGO_DIAS;

const DrilldownUuidDuplicadosParams = z
    .object({
        ...
        limit:            z.coerce.number().int().min(1).max(2000).default(UUID_DUP_PAGE_SIZE),
    })
    .superRefine((v, c) => refineFiscalNominaDateRange(v, c, MAX_RANGO_DIAS));

type UuidDuplicadoRow = {
...
export async function GET(req: NextRequest) {
    try {
        ...
        const ipRl = await rateLimit(`fn-drilluuid-ip:${sourceIp}`, { limit: 60, interval: 60_000 });
        ...
        const userRl = await rateLimit(`fn-drilluuid-user:${session.user.id}`, { limit: 40, interval: 60_000 });
        ...
        const parsed = DrilldownUuidDuplicadosParams.safeParse(raw);
        ...
        const orgRl = await rateLimit(`fn-drilluuid-org:${ctx.organizationId}`, { limit: 180, interval: 60_000 });
        ...
        const limit    = Math.min(parsed.data.limit, UUID_DUP_PAGE_SIZE);
        ...
        // ===== 7. $queryRaw · CTE UUID dups + Window Function # apariciones + Keyset =====
        // Paso 1 CTE: identificar UUIDs repetidos (GROUP + HAVING COUNT(*)>1)
        // Paso 2 JOIN de vuelta para recuperar cada CFDI individual
        // Paso 3 Window COUNT(*) OVER(PARTITION BY uuid) = apariciones por fila
        // Keyset = pr.id (ASC) — igual que cancelados.
        const rawRows = await prisma.$queryRaw<Array<Record<string, unknown>>>`
            WITH uuids_dup AS (
                SELECT pr.uuid AS dup_uuid
                 ...
                 WHERE pr.company_id      = ${parsed.data.companyId}::text
                   ...
                   AND pr.uuid IS NOT NULL
                   AND length(pr.uuid) > 0
              GROUP BY pr.uuid
            ...
            SELECT pr.id                              AS receipt_id,
                   ...
                   pr.uuid                            AS uuid,
                   COUNT(*) OVER (PARTITION BY pr.uuid)::int AS apariciones_uuid,
         ...
         LEFT JOIN payroll_receptors   rec ON rec.payroll_receipt_id = pr.id
              JOIN uuids_dup           ud  ON ud.dup_uuid = pr.uuid
             WHERE pr.company_id      = ${parsed.data.companyId}::text
               AND pr.organization_id = ${orgId}::text
          ...
          ORDER BY pr.uuid ASC, pr.fecha_pago DESC, pr.id ASC
        ...
        const data: UuidDuplicadoRow[] = taken.map(r => {
            ...
            return {
                ...
                uuid:              safeStr(r.uuid),
                aparicionesUuid:   safeNum(r.apariciones_uuid, 2),
                fechaEmision:      safeIso(r.fecha_emision),
        ...
        // Conteo de UUIDs únicos duplicados (columna agregada header — útil para usuario)
        const uuidsUnicosDup = new Set(data.map(r => r.uuid)).size;
        ...
        return NextResponse.json(
            {
                ...
                scope: {
                    ...
                    filters: { startDate: startIso, endDate: endIso },
                },
                pageInfo: {
                    pageSize,
                    returned,
                ...
                totals: {
                    /** # de UUIDs únicos que tienen duplicados (el KPI real del header) */
                    countUuidsUnicosDuplicados: uuidsUnicosDup,
                    /** # total de filas devueltas (cada aparición individual) */
                    countFilas,
        ...
        console.error('[NOMINA_FISCAL_DRILLDOWN_UUIDDUP_500]', { fp: fingerprint, summary });
