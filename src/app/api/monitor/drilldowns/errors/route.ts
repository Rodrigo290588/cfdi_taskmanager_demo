...
import {
  IMPORT_RUN_SOURCES,
  IMPORT_RUN_STATUSES,
  listOrganizationImportErrorDrilldown
...
export const MonitorDrilldownErrorsQuerySchema = z.strictObject({
  orgId: zOrgIdSafe(),
...
export async function GET(request: NextRequest) {
  ...
  const rlIp = await rateLimit(`monitor:drilldown:ip:${clientIp}`, { interval: 60 * 1000, limit: 90 })
  if (!rlIp.success) {
  ...
  try {
    ...
    if (session?.user?.id) {
      const rlUser = await rateLimit(`monitor:drilldown:user:${session.user.id}`, { interval: 60 * 1000, limit: 45 })
      if (!rlUser.success) {
        return NextResponse.json(
    ...
    const rlOrg = await rateLimit(`monitor:drilldown:org:${access.organizationId}`, { interval: 60 * 1000, limit: 180 })
    if (!rlOrg.success) {
      return NextResponse.json(
    ...
    const query = MonitorDrilldownErrorsQuerySchema.parse(rawParams)
    const result = await listOrganizationImportErrorDrilldown({
    ...
    const errFp = fp32(fingerprint(`monitor_drilldown_errors:${msgHash ?? errSummary.name}:${Date.now()}`))
    console.error(`[MON-DRILLDOWN-${errFp}] Error fetching import monitor error drilldown:`, JSON.stringify(errSummary))
    return NextResponse.json(
      { error: 'No fue posible obtener el drilldown de errores del monitor', errorRef: errFp },
